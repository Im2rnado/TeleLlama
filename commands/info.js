...
module.exports = {
	...
	async execute(ctx, sessions) {
		...
		for (let i = 0; i < object.length; i++) {
			...
			if(item00.templateId.toLowerCase().includes("AthenaCharacter")) {
				const splitid = item00.templateId.split(":")[1];
				const isExlusive = exclusives.find(el => el === splitid.toLowerCase());
				if (isExlusive) {
					const item = cosmetics.find(it => it.id.toLowerCase() === splitid);
					exclusive.push(item);
				}
			}
		}
