...
module.exports = {
	...
	async execute(ctx, sessions) {
		...
		await axios.delete(`https://account-public-service-prod03.ol.epicgames.com/account/api/public/account/${token.account_id}/externalAuths/psn`, { headers: {
			"Content-Type": "application/json",
			"Authorization": `Bearer ${token.access_token}`,
		} }).then((response) => {
			console.error(response.data);

			ctx.reply("✅ Successfully Unlinked *PSN*", { parse_mode: "markdown" });
		}).catch((err) => {
			console.error(err);
